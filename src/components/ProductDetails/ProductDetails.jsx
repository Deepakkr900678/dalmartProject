...
const ProductDetails = () => {
    ...
    useEffect(() => {
        const foundProduct = productsLocal.find((item) => item._id === productId);
                            ...
                            <div className="w-full sm:w-2/5 sm:sticky top-16 sm:h-screen">
                                ...
                                <div className="flex flex-col gap-3 m-3">
                                    <div className="w-full h-full pb-6 border relative">
                                        <Slider {...settings}>
                                            {localProduct?.images && localProduct?.images.map((item, i) => (
                                                <img draggable="false" className="w-full h-96 object-contain" src={item.url} alt={localProduct?.name} key={i} />
                                            ))}
                                        </Slider>
                                    ...
                                    <div className="w-full flex gap-3">
                                        ...
                                        <button onClick={buyNow} disabled={product?.stock < 1 ? true : false} className={product?.stock < 1 ? "p-4 w-full flex items-center justify-center gap-2 text-white bg-red-600 cursor-not-allowed rounded-sm shadow hover:shadow-lg" : "p-4 w-1/2 flex items-center justify-center gap-2 text-white bg-primary-orange rounded-sm shadow hover:shadow-lg"}>
                                            <FlashOnIcon />
                                            {localProduct?.stock < 1 ? "OUT OF STOCK" : "BUY NOW"}
                                        </button>
                                        {/* <!-- add to cart btn --> */}
                                    </div>
                            ...
                            <div className="flex-1 py-2 px-3">
                                ...
                                <div className="flex flex-col gap-2 mb-4">
                                    ...
                                    <div className="flex items-baseline gap-2 text-3xl font-medium">
                                        <span className="text-gray-800">₹{localProduct?.price}</span>
                                        <span className="text-base text-gray-500 line-through">₹{localProduct?.price?.toLocaleString()}</span>
                                        <span className="text-base text-primary-green">{localProduct?.price?.toLocaleString()}%&nbsp;off</span>
                                    </div>
